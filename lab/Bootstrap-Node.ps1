...
& "$PSScriptRoot\Install-DscEncryptionCertificate.ps1" -ComputerName $ComputerName -Credential $MyConfiguration.VmAdministratorCredential -CertificatePfxPath "$env:TEMP\DscPrivateKey.pfx" -CertificatePfxPassword $MyConfiguration.DscEncryptionCertPfxPassword
